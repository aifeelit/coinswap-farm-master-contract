...
contract('MasterCSS', ([alice, bob, carol, dev, minter]) => {
  ...
  it('real case', async () => {
    ...
    await this.master.add('500', this.lp4.address, true, { from: minter });
    ...
    assert.equal(
      (await this.css.balanceOf(this.lottery.address)).toString(),
      '0'
    );
    ...
    assert.equal((await this.css.balanceOf(carol)).toString(), '5');
  });
  ...
  it('setReceiver', async () => {
    ...
    assert.equal((await this.css.balanceOf(carol)).toString(), '7');
    await this.lottery.setReceiver(alice, { from: dev });
    assert.equal((await this.lottery.pendingReward('1')).toString(), '7');
    await this.lottery.harvest(1, { from: dev });
    assert.equal((await this.css.balanceOf(alice)).toString(), '15');
  });
